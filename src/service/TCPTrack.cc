...
/*
 * every kind of packet generation by an external classes (plugin and scramble)
 * collects the packets in a vector<Packet *> as public member of the class.
 *
...
uint32_t TCPTrack::acquirePktVector(Packet &origpkt, vector<Packet *> &srcpV, const char *info, queue_t wtf)
{
    ...
    /* clear the vector, some of the packets are now moved, some others just freed) */
    srcpV.clear();
...
bool TCPTrack::notifyIncoming(Packet &origpkt)
{
    ...
    origpkt.SELFLOG("orig pkt: before incoming incoming mangling");
#endif
...
#ifdef ENABLE_INCOMING_DEBUG
    origpkt.SELFLOG("orig pkt: after incoming mangling");
#endif
...
bool TCPTrack::injectHack(Packet &origpkt)
{
    ...
    /* XXX FIXME:
     * SessionTrack will need to take into account the usable scrambles for a destination
     * Actually we use a puppet mask to handle this.
     */
    ...
    /* SELECT APPLICABLE HACKS, the selection are based on:
     * 1) the plugin/hacks detects if the condition exists (eg: the hack wants a SYN and the packet is a RST+ACK)
     * 2) compute the percentage: mixing the hack-choosed and the user-choosed  */
    for (vector<PluginTrack*>::iterator it = plugin_pool->pool.begin(); it != plugin_pool->pool.end(); ++it)
    {
        PluginTrack *pt = *it;
...
bool TCPTrack::lastPktFix(Packet &pkt)
{
    ...
    retval = scramble->mystifyScramble(AFTER_CHECKSUM, pkt);
...
void TCPTrack::handleYoungPackets(void)
{
    ...
    for (p_queue.select(YOUNG); ((pkt = p_queue.get()) != NULL);)
    {
        ...
        case NETWORK:
            ...
            ++(sessiontrack_map->get(*pkt).incoming.natural);
